   ...
   return (
     <div className='w-full  px-4 sm:px-16 md:px-28 2xl:px-48 py-10 flex flex-col gap-10 text-center items-center '>
        ...
        {page.map((item , idx) => (
            ...
            <div className={`w-full bg-white  z-50 rounded-2xl py-4 px-8 transition-all flex justify-between  '}`}>
                <h1 className='font-bold'>Q: {item.question}</h1>
                <FaPlus style={{transform : idx == showKeyIndex ? 'rotateZ(45deg)' : ''} } className='transition-all duration-300' onMouseDown={() => toggleShowKeyDetail(idx)} />
            </div>
            <AnimatePresence>
            {idx == showKeyIndex && (
                    <motion.p
