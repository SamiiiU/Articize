    ...
    <h1 className='2xl:text-[3rem] text-[2.5rem] font-[800]' id='headingHero'
     ...
     {page.platforms.map((platform , index) => (
        <div key={index} className='sm:w-3/4 w-full  flex flex-col sm:px-10 h-fit '>
            <h1 className='font-bold text-4xl  rounded-t-xl py-2 mb-2 text-[#013a6e] sm:text-center text-left'>{platform.heading}</h1>

            <span className='w-full  my-4  py-2 rounded-xl flex sm:flex-row flex-col items-center gap-x-10'>
                    <div className='sm:w-1/2 w-full  text-left  text-xl font-bold'>
                    {platform.items.map((company , idx ) => (  
                      <p className='my-2' key={idx}>{company.itemName}</p>
                    ))}
                    </div>
                    <div className='sm:min-w-1/2  w-full flex justify-center h-fit items-center ' >
                        {platform.items.map((image , idx) => (
                          <span key={idx} className={`rounded-full sm:overflow-hidden  flex justify-center items-center sm:p-6 bg-white sm:border-2 border-black  sm:w-32 sm:h-32 w-20 h-20 ${idx % 2 == 0 ? 'translate-y-10' : ''}`}>
