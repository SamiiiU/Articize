    ...
    return (
        <div className='w-full  px-4 sm:px-16 md:px-28 2xl:px-80 py-10 flex flex-col gap-10 text-center '>
            ...
            {scrwidth > 1000 ? (
                ...
                <div className='w-full flex flex-col text-left gap-y-8'>
                    {HomeData.Power.map((item, index) => (
                        <div key={index} className='w-full flex flex-col p-6 rounded-xl shadow-md'>
                            <h1 className='font-semibold text-xl  mb-4 '>{item.container.heading}</h1>
                            <p className='mb-8'>{item.container.description}</p>

                            <h1 className='font-semibold text-3xl text-orange-500'>Outcomes</h1>
                            {item.container.points.map((outcome, idx) => (
                                <div className='flex gap-x-4 text-left items-center mb-2' key={idx}>
                                    {/* <SiTicktick  className='text-orange-500 text-sm'/> */}
                                    <p className='flex-1 text-sm'>{outcome}</p>
                                </div>
